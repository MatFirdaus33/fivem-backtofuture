...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (GetEntityModel(veh) == GetHashKey("dune2")) then

        	-- silly UI
        	drawRct(0.01, 0.4, 0.027, 0.25, 0, 0, 0, 150)
        	drawRct(0.012, 0.402, 0.01, (0.245/100)*time_charge, 0, 0, 250, 150)
        	drawRct(0.025, 0.402, 0.01, (0.245/1000)*cooldown, 150, 0, 0, 150)

        	if (not boosting and time_charge >= 100) then
        		local pos = GetBlipInfoIdCoord(GetFirstBlipInfoId(8))
        		if (pos.x ~= 0.0) then
	            	draw2dText("~g~[GO TO FUTURE TEKAN ENTER]", { 0.43, 0.8 } )
	            	if (IsControlJustPressed(1, 191)) then
	            		boosting = true
	            		StartBoost(veh)
	            	end
	            else
	            	draw2dText("~r~[SILA TANDA KAWASAN TERDEKAT ANDA UNTUK GO TO FUTURE]", { 0.35, 0.8 } )
	            end
            end

            if (GetEntitySpeed(veh)*2.10201 >= 10.0 and cooldown <= 0) then
            	if (time_charge < 100) then
